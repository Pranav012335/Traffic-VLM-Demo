...
def _launch_demo(args, model, processor, backend):
    ...
    def create_predict_fn():
        ...
        def predict(_chatbot, task_history):
            ...
            if len(_chatbot) > 0 and isinstance(_chatbot[-1], dict):
                 # Gradio 5 text extraction form User Dict
                 chat_content = _chatbot[-1].get('content', '')
                 chat_query = chat_content if isinstance(chat_content, str) else ''
            else:
                 chat_query = _chatbot[-1][0] if len(_chatbot) > 0 else ''
            
            query = task_history[-1][0]
            if len(chat_query) == 0 and not isinstance(query, tuple):
                _chatbot.pop()
                if len(task_history) > 0:
                    task_history.pop()
                return _chatbot
            
            # Print the text query instead of tuple objects
            ...
            for q, a in history_cp:
                if isinstance(q, (tuple, list)):
                    # File paths from tuples
                    file_path = q[0]
                    if _is_video_file(file_path):
                        content.append({'video': f'{os.path.abspath(file_path)}'})
                    else:
                        content.append({'image': f'{os.path.abspath(file_path)}'})
                else:
                    # Text prompts
            ...
            if len(messages) > 0 and messages[-1]['role'] == 'assistant' and messages[-1]['content'][0].get('text') == 'None':
                messages.pop()

            for response in call_local_model(model, processor, messages, backend):
                if len(_chatbot) > 0 and isinstance(_chatbot[-1], dict) and _chatbot[-1].get("role") == "user":
                    _chatbot.append({"role": "assistant", "content": _remove_image_special(_parse_text(response))})
                else:
                    _chatbot[-1] = {"role": "assistant", "content": _remove_image_special(_parse_text(response))}
    ...
    def add_text(history, task_history, text):
        ...
        history = history + [{"role": "user", "content": _parse_text(text)}]
    ...
    def add_file(history, task_history, file):
        ...
        # Gradio 5 Chatbot File payload
        history = history + [{"role": "user", "content": gr.FileData(path=file_path)}]
...
<center><font size=3>This WebUI is based on Qwen3-VL, developed by Alibaba Cloud. Backend: {backend.upper()}</center>""")
        ...
        # Adding a warning notice since the repository is for Qwen3-VL but we are running a Qwen2-VL model checkpoint.
        gr.Markdown("<center><font size=2 color='red'>Note: Running Qwen2-VL-2B locally behind the scenes.</font></center>")
